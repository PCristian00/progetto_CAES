...
void JuceSynthFrameworkAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < mySynth.getNumVoices(); i++)
    {
        ...
        {
            float attack = tree.getRawParameterValue("attack")->load();
float decay = tree.getRawParameterValue("decay")->load();
float sustain = tree.getRawParameterValue("sustain")->load();
float release = tree.getRawParameterValue("release")->load();
myVoice->getEnvelopeParams(&attack, &decay, &sustain, &release);
            
            float wavetype = tree.getRawParameterValue("wavetype")->load();
float wavetype2 = tree.getRawParameterValue("wavetype2")->load();
float filterType = tree.getRawParameterValue("filterType")->load();
float filterCutoff = tree.getRawParameterValue("filterCutoff")->load();
float filterRes = tree.getRawParameterValue("filterRes")->load();
float mastergain = tree.getRawParameterValue("mastergain")->load();
float blend = tree.getRawParameterValue("blend")->load();
float pbup = tree.getRawParameterValue("pbup")->load();
float pbdown = tree.getRawParameterValue("pbdown")->load();

myVoice->getOscType(&wavetype);
myVoice->getOsc2Type(&wavetype2);
myVoice->getFilterParams(&filterType, &filterCutoff, &filterRes);
myVoice->getWillsParams(&mastergain, &blend, &pbup, &pbdown);
        }
    }
